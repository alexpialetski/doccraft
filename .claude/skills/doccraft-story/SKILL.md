...
- When closing a story: update `docs/queue.md` and the **Status**
...
1. **Unique `id`** — scan all `docs/stories/*.md` frontmatter and
...
- The story file exists at the correct path (`docs/stories/<slug>.md`).
...
- If the story was closed (`status: done`), `docs/queue.md` and
...
- **In prose, tables, and Notes** — whenever you *write* a reference a reader
  ...
  resolvable file path: `[pipeline/P1.30](../services/pipeline/docs/stories/p1-...md)`.
...
- When closing a story (`status: done`), update `docs/queue.md`
