...
1. **Next number** — scan all `docs/adr/NNN-*.md` files and pick
...
- The ADR file exists at the correct path (`docs/adr/NNN-slug.md`)
...
- If superseding: the old ADR's status line has been updated in the same
  turn.
- `docs/adr/README.md` index has been updated with the new entry.
...
- After creating an ADR that drives implementation, create a story under
  `docs/stories/` with `adr_refs` pointing to this ADR (use
...
- After creating or superseding, update `docs/adr/README.md`
