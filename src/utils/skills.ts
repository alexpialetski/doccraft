...
const SCHEMA_JSON = JSON.stringify(DOCCRAFT_CONFIG_SCHEMA, null, 2);
...
export const TEMPLATES_ROOT_CONFIG = path.join(PACKAGE_ROOT, 'templates', 'doccraft.json');
...
export const MANAGED_HEADER =
  ...
  'Local edits will be overwritten. See `doccraft.json` to override ' +
...
/**
 * Reads `docsDir` from `doccraft.json` at the project root. Defaults to
 ...
 * — Claude Code reads it natively, Cursor 2.4+ auto-discovers it, and every
 * major discovery implementation points there. doccraft always writes skills
 * here regardless of which tools the user selected; `.cursor/skills/` is
 * never written by doccraft (Cursor would load every skill twice without
 * dedupe, wasting context window).
 *
 * `.cursor/rules/*.mdc` stubs are a separate Cursor-native primitive (ADR
 * 003) and are still written when Cursor is in the user's selection.
 */
export function getCanonicalSkillsTool(): SkillTool {
  const claude = SUPPORTED_TOOLS.find((t) => t.id === 'claude');
  if (!claude) {
    throw new Error('doccraft internal error: claude tool missing from SUPPORTED_TOOLS');
  }
  return claude;
}
 ...
 *
 * For `doccraft-config`, the `{{DOCCRAFT_CONFIG_SCHEMA}}` placeholder is
 * replaced with the embedded JSON Schema so the skill is self-contained.
 */
...
/**
 * Writes `doccraft.json` to the project root if it does not already exist.
 ...
 *
 * Substitutes `{{DOCCRAFT_VERSION}}` with the current package version so
 * the `version` stamp and `$schema` URL are correct on first scaffold.
 */
export async function scaffoldRootConfigIfMissing(projectPath: string): Promise<boolean> {
  const target = path.join(projectPath, 'doccraft.json');
  if (existsSync(target)) return false;
  if (!existsSync(TEMPLATES_ROOT_CONFIG)) return false;
  const raw = await readFile(TEMPLATES_ROOT_CONFIG, 'utf8');
  const body = raw.replaceAll('{{DOCCRAFT_VERSION}}', PACKAGE_VERSION);
  await writeFile(target, body, 'utf8');
  return true;
}

/**
 * Performs a surgical edit on `doccraft.json` to update the `"version"` value
 * and the version segment of the `"$schema"` jsDelivr URL. Every other byte
 * in the file (key order, whitespace, user values) is preserved.
 *
 * Returns `true` if the file was found and modified, `false` if not found.
 */
export async function bumpConfigVersion(projectPath: string, newVersion: string): Promise<boolean> {
  const configPath = path.join(projectPath, 'doccraft.json');
  ...
  content = content.replace(
    /(https:\/\/cdn\.jsdelivr\.net\/npm\/doccraft@)[^\\/"]+(\/schema\/doccraft\.schema\.json)/,
    `$1${newVersion}$2`
  );
